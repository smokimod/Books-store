...
export const RegStepOne = ({ register, errors, status, password }) => {
  const [showPassword, setShowPassword] = useState(false);

  const styleErrorUserName =
    errors?.username || status ? { borderBottom: "solid 1px red" } : null;

  const styleErrorPassword =
    errors?.password || status ? { borderBottom: "solid 1px red" } : null;
  ...
  return (
    <React.Fragment>
      <label htmlFor="reg-login-input">
        ...
        <div
          style={errors.username ? { color: "red" } : null}
          data-test-id=" hint"
        >
